package com.yufeng.authorization.interceptor;
...
public class AuthorizationInterceptor extends HandlerInterceptorAdapter {
    ...
    public boolean preHandle(HttpServletRequest request,
        ...
        //验证token
        TokenModel model = tokenModelService.getToken(authorization);
        if (tokenModelService.checkToken(Constants.AUTHORIZATION)) {
            //如果token验证成功，将token对应的用户id存在request中，便于之后注入
            request.setAttribute(Constants.CURRENT_USER_ID, model.getRegisterAccountId());
            return true;
        }
